...
void MGFitnessCodonSubMatrix::ComputeStationary()	{
  ...
  for (int i=0; i<GetNstate(); i++)	{
    mStationary[i] = NucMatrix->Stationary(GetCodonPosition(0,i)) * NucMatrix->Stationary(GetCodonPosition(1,i)) * NucMatrix->Stationary(GetCodonPosition(2,i)) * GetFitness(GetCodonStateSpace()->Translation(i));
    total += mStationary[i];
  }
  ...
  for (int j=0; j<GetNstate(); j++)	{
    if (i!=j)	{
      ...
      if ((pos != -1) && (pos != 3))	{
        ...
        Q[i][j] = (*NucMatrix)(a,b);
        if (! Synonymous(i,j))	{
          Q[i][j] *=  sqrt((GetFitness(GetCodonStateSpace()->Translation(j)))/(GetFitness(GetCodonStateSpace()->Translation(i))));
        }
      }
      else	{
        Q[i][j] = 0;
...
void MGFitnessCodonUsageSubMatrix::ComputeStationary()	{
  ...
  double total = 0;
  for (int i=0; i<GetNstate(); i++)	{
    mStationary[i] = NucMatrix->Stationary(GetCodonPosition(0,i)) * NucMatrix->Stationary(GetCodonPosition(1,i)) * NucMatrix->Stationary(GetCodonPosition(2,i)) * GetFitness(GetCodonStateSpace()->Translation(i)) * GetCodonUsageSelection(i);
    total += mStationary[i];
  }
  ...
  for (int j=0; j<GetNstate(); j++)	{
    if (i!=j)	{
      ...
      if ((pos != -1) && (pos != 3))	{
        ...
        Q[i][j] = (*NucMatrix)(a,b) * sqrt(GetCodonUsageSelection(j)/GetCodonUsageSelection(i)) ;
        if (! Synonymous(i,j))	{
          Q[i][j] *= sqrt(((GetFitness(GetCodonStateSpace()->Translation(j)))/(GetFitness(GetCodonStateSpace()->Translation(i)))));
...
//when the fitness profile and codonusageselection are from Normal or gamma // phenomenological square root model
void MGSRFitnessNormalCodonUsageSubMatrix::ComputeStationary()	{

  // compute stationary probabilities
  double total = 0;
  for (int i=0; i<GetNstate(); i++)	{
    mStationary[i] = NucMatrix->Stationary(GetCodonPosition(0,i)) * NucMatrix->Stationary(GetCodonPosition(1,i)) * NucMatrix->Stationary(GetCodonPosition(2,i)) * GetFitness(GetCodonStateSpace()->Translation(i)) * exp(GetCodonUsageSelection(i));
  ...
  for (int j=0; j<GetNstate(); j++)	{
    if (i!=j)	{
      ...
      if ((pos != -1) && (pos != 3))	{
        ...
        Q[i][j] = (*NucMatrix)(a,b) * sqrt(exp(GetCodonUsageSelection(j)-GetCodonUsageSelection(i))) ;
        if (! Synonymous(i,j))	{
          Q[i][j] *= sqrt(((GetFitness(GetCodonStateSpace()->Translation(j)))/(GetFitness(GetCodonStateSpace()->Translation(i)))));
        }
      }
  ...
  for (int j=0; j<GetNstate(); j++)	{
    if (i!=j)	{
      ...
      if ((pos != -1) && (pos != 3))	{
        ...
        double deltaS;
        if (! Synonymous(i,j))	{
          deltaS = log((GetFitness(GetCodonStateSpace()->Translation(j)))/(GetFitness(GetCodonStateSpace()->Translation(i)))) + log(GetCodonUsageSelection(j)/GetCodonUsageSelection(i));
        }
        else	{
    ...
    if (i!=j)	{
      ...
      if ((pos != -1) && (pos != 3))	{
        ...
        if(isinf(Q[i][j]))	{
          ...
          cerr << log(GetFitness(GetCodonStateSpace()->Translation(i))) - log(GetFitness(GetCodonStateSpace()->Translation(j))) << '\n';
