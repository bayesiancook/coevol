...
class CovMatrix : public BaseType, public Multiplicative {
  ...
  public:
    ...
    static double GetMaxDiagError() { return maxerror; }
    ...
    void SetOrthoNormal(bool in) { orthonormal = in; }
  ...
  public:
        ...
        for (int i = 0; i < GetDim(); i++) {
            if (i != index) {
                ...
                for (int j = 0; j < GetDim(); j++) {
                    if (j != index) {
                        m[k][l] =
                            value[i][j] - value[i][index] * value[j][index] / value[index][index];
                        l++;
                    }
                }
                k++;
    ...
    */
    ...
    void CorruptDiag() { diagflag = false; }
