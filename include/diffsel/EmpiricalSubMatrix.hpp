...
class EmpiricalSubMatrix : public virtual SubMatrix {
  public:
    ...
    void AddPath(BranchSitePath* inpath) { pathlist.push_back(inpath); }
...
class EmpiricalGTRSubMatrix : public EmpiricalSubMatrix {
  public:
    EmpiricalGTRSubMatrix(int Nstate, bool innormalise = false)
        : SubMatrix(Nstate, innormalise), EmpiricalSubMatrix(Nstate, innormalise) {
  ...
  protected:
    int GetNrr() { return Nstate * (Nstate - 1) / 2; }
