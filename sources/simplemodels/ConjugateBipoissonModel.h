...
class ConjugateBipoissonModel : public ProbModel	{
	...
	ConjugateGamma** lengths;
	ConjugateBipoisson*** counts;


	ConjugateBipoissonModel(string datafile)	{
	...
	ConjugateBipoissonModel(int inN, int inP)	{
		MakeNewModel(inN, inP);
	}
	...
	void MakeNewModel(int inN, int inP)	{
		...
		for (int i=0; i<N; i++)	{
			// rates[i] = new ConjugateGamma(alpha,alpha);
			rates[i] = new ConjugateGamma(alpha,alpha);
		}

		lengths = new ConjugateGamma*[P];
		for (int j=0; j<P; j++)	{
			lengths[j] = new ConjugateGamma(lambda,mu);
		}

		counts = new ConjugateBipoisson**[N];
		for (int i=0; i<N; i++)	{
			counts[i] = new ConjugateBipoisson*[P];
			for (int j=0; j<P; j++)	{
				counts[i][j] = new ConjugateBipoisson(lengths[j],rates[i]);
			}
		}
	...
	double Move(double tuning)	{
		...
		*/
		...
		for (int j=0; j<P; j++)	{
			lengths[j]->InactivateSufficientStatistic();
		}
		return 1;
	}
