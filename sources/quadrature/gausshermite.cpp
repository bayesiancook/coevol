...
double f(double alpha, double beta, double x)	{
	...
	return 1.0 / (1 + alpha * exp(-beta * x));
}

int main(int argc, char* argv[])	{

	int maxorder = atoi(argv[1]);
	int maxorder2 = atoi(argv[2]);
	int zscore = atoi(argv[3]);
	...
	for (int order = maxorder/10; order<= maxorder; order+=maxorder / 10)	{
		...
		double* x = new double[2* order+1];
		
		double total = 0;
		for (int i=order; i>0; i--)	{
			double x = ((double) zscore * i) / order;
			total += f(alpha,beta,x) * exp(-x*x);
			total += f(alpha,beta,-x) * exp(-x * x);
		}
		total += f(alpha,beta,0);
		total /= (2 * order + 1);
		total *= 2 * zscore; 
		cout.precision(20);
		cout << order << '\t' << total << '\t' << sqrt(3.141592653589793238)   << '\n';
		k1 = total;

		delete[] x;
	}

	for (int order = 2; order<= maxorder2; order+=2)	{
		...
		for (int i=0; i<order/2; i++)	{
			total += w[i] * f(alpha,beta,x[i]);
			total += w[order - 1 - i] * f(alpha,beta,x[order - 1 - i]);
		}
		cout.precision(20);
		// cout << order << '\t' << total << '\n';
		cout << order << '\t' << total << '\t' << sqrt(3.141592653589793238)   << '\n';
