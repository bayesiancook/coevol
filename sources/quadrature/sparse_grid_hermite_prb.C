...
//
{
  ...
  cout << "  C++ version\n";
  cout << "\n";
  cout << "  Test the routines in the SPARSE_GRID_HERMITE library.\n";
//
...
for (int level_max = min; level_max <= max; level_max++)	{
...
//
  ...
  dtable_write0 ( r_filename, dim_num, 2,         r );
  dtable_write0 ( w_filename, 1,       point_num, w );
  dtable_write0 ( x_filename, dim_num, point_num, x );
