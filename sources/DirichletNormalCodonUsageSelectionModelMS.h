...
class NormalNormalCompMove : public MCUpdate	{
	...
	NormalNormalCompMove(IIDNormalIIDArray* indiff1, IIDNormalIIDArray* indiff2, double intuning, int inn, int innrep)	{
		diff1 = indiff1;
		diff2 = indiff2;
		tuning = intuning;
		n = inn;
...
class RenormalizedIIDStat : public Dvar<Profile> {
	...
	// the constructor takes 4 arguments: pointers to the 3 variables a and b and c and the variable beta
	...
	// the only function that we need to define is the one that makes multiplication and renormalization
	void specialUpdate()	{
	...
	DirichletNormalCodonUsageSelectionModelMS(string datafile, string treefile, int inK, string intype, int inconjugate, string inmechanism, bool sample = true) {
		...
		*/
		...
		for (int k=0; k<K; k++)	{
			...
			for (int i=0; i<Nsite;i++){
				if (K==1){
					statarray[k][i] = new RenormalizedIIDStat(globalselectionprofile->GetVal(i), zero, zero, beta);
				}
				else{
					if(k == 0){
						statarray[k][i] = new RenormalizedIIDStat(globalselectionprofile->GetVal(i), zero, zero, beta);
						...
						statarray[k][i] = new RenormalizedIIDStat(globalselectionprofile->GetVal(i), selectionnormal[1]->GetVal(i), zero, beta);
					}

					else if(k == 2){
						statarray[k][i] = new RenormalizedIIDStat(globalselectionprofile->GetVal(i),selectionnormal[1]->GetVal(i), selectionnormal[2]->GetVal(i), beta);
					}

					else if(k == 3){
						statarray[k][i] = new RenormalizedIIDStat(globalselectionprofile->GetVal(i),selectionnormal[1]->GetVal(i), selectionnormal[3]->GetVal(i), beta);
					}
				}
			}
		}
...
//Square Root //phenimenological
		if(mechanism == "SR")	{
			...
			for (int k=0; k<K; k++)	{
				...
				for (int i=0; i<Nsite;i++){
					submatrix[k][i] = new RandomMGSRFitnessCodonUsageSubMatrix((CodonStateSpace*) codondata->GetStateSpace(), nucmatrix, statarray[k][i], codonusageselection,false);
				}
			}
		}
...
//Mutation Selection // mechanistic
		if(mechanism == "MS")	{
			...
			for (int k=0; k<K; k++)	{
				submatrix[k] = new RandomSubMatrix* [Nsite];
				for (int i=0; i<Nsite;i++){
					submatrix[k][i] = new RandomMGMSFitnessCodonUsageSubMatrix((CodonStateSpace*) codondata->GetStateSpace(), nucmatrix, statarray[k][i], codonusageselection,false);
				}
			}
		}
	...
	void MakeScheduler()	{
		...
		for( int n=0;n<3;n++){
			...
			for (int m=0; m<20; m++)	{
				...
				scheduler.Register(new ComplexDirichletIIDArrayMove(globalselectionprofile,0.15,10),1,"global stat");

				/*
				scheduler.Register(new DirichletNormalCompMove(globalselectionprofile,selectionnormal[1],0.03,1,5),1,"global diff comp");
				scheduler.Register(new DirichletNormalCompMove(globalselectionprofile,selectionnormal[1],0.01,3,5),1,"global diff comp");
				scheduler.Register(new DirichletNormalCompMove(globalselectionprofile,selectionnormal[1],0.001,10,5),1,"global diff comp");
				*/
				...
				for(int i=1;i<K;i++){
					...
					scheduler.Register(new IIDNormalIIDArrayMove(selectionnormal[i],5,1),10,"stat" + indice);
					scheduler.Register(new IIDNormalIIDArrayMove(selectionnormal[i],3,5),10,"stat" + indice);
					scheduler.Register(new IIDNormalIIDArrayMove(selectionnormal[i],1,10),10,"stat" + indice);
					scheduler.Register(new IIDNormalIIDArrayMove(selectionnormal[i],1,20),10,"stat" + indice);
				}
...
//cerr << " selectionnormal : " << *selectionnormal[1]<< '\n';
	...
	void PostPredAli(string name, int sitemin, int sitemax)	{
		...
		if (sitemax != -1)	{
			CodonSequenceAlignment* datacopy2 = new CodonSequenceAlignment(datacopy,sitemin,sitemax);
