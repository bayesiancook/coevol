...
#include "Sample.h"
#include "AAProfileMutSelMatInfMixModel.h"
//#include "NeffAAProfileInfMixMutSelModel.h"
//#include "MeanValTree.h"

class AAProfileMutSelInfMixSample : public Sample	{
	...
	AAProfileMutSelMatInfMixModel* GetModel() {return (AAProfileMutSelMatInfMixModel*) model;}
	//AAProfileInfMixMutSelModel* GetModel() {return (NeffAAProfileInfMixMutSelModel*) model;}

	AAProfileMutSelInfMixSample(string filename, int inburnin, int inevery, int inuntil) : Sample(filename,inburnin,inevery,inuntil)	{
		Open();
	}
	...
	void Open()	{
		...
		if (modeltype == "AAPROFILEINFMIXMUTSEL")	{
			model = new AAProfileMutSelMatInfMixModel(datafile,treefile,P,true,type);
	...
	void Read()	{
		...
		double** meanAAFitness = new double*[Nsite];
		double* tempAAProfile = new double[Naa];
		for (int i=0; i<Nsite; i++)	{
			meanAAProfile[i] = new double[Naa];
			meanAAFitness[i] = new double[Naa];
			for (int j=0; j<Naa; j++)	{
				meanAAProfile[i][j] = 0;
				meanAAFitness[i][j] = 0;
			}
		...
		for (int i=0; i<size; i++)	{
			...
			for (int site=0; site<Nsite; site++)	{
				for (int aa=0; aa<Naa; aa++)	{
					...
					meanAAFitness[site][aa] += log(tempAAProfile[aa]);
				}
			}
		...
		ofstream osfit((name + ".ssfitness").c_str());
		os << Nsite << '\n';
		osfit << Nsite << '\n';
		for (int site=0; site<Nsite; site++)	{
			double shift = 0;
			for (int aa=0; aa<Naa; aa++)	{
				...
				meanAAFitness[site][aa] /= size;
				os << meanAAProfile[site][aa] << '\t';
				//osfit << meanAAFitness[site][aa] << '\t';
				shift += meanAAFitness[site][aa];
			}
			os << '\n';
			...
			for (int aa=0; aa<Naa; aa++)	{
				osfit << (meanAAFitness[site][aa] - shift) << '\t';
			}
	...
	void OutputTrees()	{
		...
		//MeanExpNormTree* meantree = new MeanExpNormTree(GetModel()->GetTree());
		//meantree->Reset();

		ofstream trees_os((name + ".trees").c_str());

		// cycle over the sample
		for (int i=0; i<size; i++)	{
			GetNextPoint();
			GetModel()->Update();
			GetModel()->PrintGammaTree(trees_os);
			//cout << *GetModel()->AAProfileConcentration << '\n';
			//cout << *GetModel()->
		}
...
int main(int argc, char* argv[])	{
	...
	AAProfileMutSelInfMixSample sample(name,burnin,every,until);
	if (postpred)	{
