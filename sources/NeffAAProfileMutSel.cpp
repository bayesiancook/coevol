...
class NeffAAProfileMutSelChain : public Chain	{
	...
	NeffAAProfileMutSelChain(string indata, string intree, int inP, string filename, GeneticCodeType intype, string inprofilefile, int force = 1)	{
		modeltype = "NEFFAAPROFILEMUTSEL";
		datafile = indata;
		treefile = intree;
		type = intype;
		P = inP;
	...
	NeffAAProfileMutSelChain(string filename)	{
		name = filename;
		Open();
	}
	...
	void New(int force)	{
		if (modeltype == "NEFFAAPROFILEMUTSEL")	{
			model = new NeffAAProfileMutSelModel(datafile,treefile,P,true,type,profilefile);
		}
		else	{
	...
	void Open()	{
		...
		if (modeltype == "NEFFAAPROFILEMUTSEL")	{
			model = new NeffAAProfileMutSelModel(datafile,treefile,P,false,type,profilefile);
		}
		else	{
	...
	void Gnuplot()	{
		...
		gp_os << "p '" << (name + ".trace").c_str() << "' u 4 title 'effsize'\n" << "pause -1\n";
...
int main(int argc, char* argv[])	{
	...
	NeffAAProfileMutSelChain* chain;
	if (datafile == "")	{
		chain = new NeffAAProfileMutSelChain(name);
	}
	else	{
		chain = new NeffAAProfileMutSelChain(datafile,treefile,P,name,type,profilefile);
	}
