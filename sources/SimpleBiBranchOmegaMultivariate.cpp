...
#include "Chain.h"
#include "SimpleBiBranchOmegaMultivariateModel.h"
...
class BranchOmegaMultivariateChain : public Chain	{
	...
	BranchOmegaMultivariateChain(string indata1, string indata2, string intree, string incontdata, string incalibfile, double inrootage, double inrootstdev, double inpriorsigma, int indf, int incontdatatype,bool inclamptree, bool inmeanexp, int innrep, string filename, int force = 1)	{
	...
	void New(int force)	{
		if (modeltype == "CONJUGATEBRANCHOMEGAMULTIVARIATE")	{
			model = new BranchOmegaMultivariateModel(datafile1,datafile2,treefile,contdatafile,calibfile,rootage,rootstdev,priorsigma,df,contdatatype,clamptree,meanexp,nrep,true);
		}
		else	{
	...
	void Open()	{
		...
		is >> modeltype;
		is >> datafile1 >> datafile2 >> treefile >> contdatafile;
	...
	void Save()	{
		...
		param_os << datafile1 << '\t' << datafile2 << '\t' << treefile << '\t' << contdatafile << '\n';
...
int main(int argc, char* argv[])	{
	...
	if (argc == 2)	{
		...
		BranchOmegaMultivariateChain* chain = new BranchOmegaMultivariateChain(datafile1,datafile2,treefile,contdatafile,calibfile,rootage,rootstdev,priorsigma,df,contdatatype,clamptree,meanexp,nrep,name,force);
