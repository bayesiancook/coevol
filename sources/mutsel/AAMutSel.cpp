...
class AAMutSelChain : public Chain	{
	...
	AAMutSelChain(string indata, string intree, int inP, string filename, GeneticCodeType intype, int force = 1)	{
		modeltype = "AAMUTSEL";
		datafile = indata;
		treefile = intree;
		type = intype;
	...
	void Open()	{
		...
		if (modeltype == "AAMUTSEL")	{
			model = new AAMutSelModel(datafile,treefile,P,false,type);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	if (argc == 2)	{
		string name = argv[1];
		AAMutSelChain* chain = new AAMutSelChain(name);
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
	...
	// this is a new chain
		...
		AAMutSelChain* chain = new AAMutSelChain(datafile,treefile,P,name,type);
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
	}
