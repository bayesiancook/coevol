...
#ifndef MGAAMUTSELCODONSUBMATRIX_H
#define MGAAMUTSELCODONSUBMATRIX_H
...
class MGAAMutSelCodonSubMatrix : public MGCodonSubMatrix	{
	...
	void SetAAScaledLogFitness(double* inaa)	{
		for (int j=0; j<Naa; j++)	{
			aascaledlogfitness[j] = popsize * inaa[j];
		}	
		double max = aascaledlogfitness[0];
		for (int j=1; j<Naa; j++)	{
			if (max < aascaledlogfitness[j])	{
				max = aascaledlogfitness[j];
			}
		}
		for (int j=0; j<Naa; j++)	{
			expaascaledlogfitness[j] = exp(aascaledlogfitness[j] - max);
		}
	}
...
class RandomMGAAMutSelCodonSubMatrix : public MGAAMutSelCodonSubMatrix, public RandomCodonSubMatrix	{
	...
	RandomMGAAMutSelCodonSubMatrix(CodonStateSpace* instatespace, RandomSubMatrix* inmatrix, Var<RealVector>* inAAScaledLogFitness, Var<RealVector>* inMu = 0, Var<Real>* inLogPopSize = 0, Var<PosReal>* inPopSize = 0) :
			SubMatrix(instatespace->GetNstate()),
