...
class GCPopSizeAAReducedMutSelSample : public Sample	{
	...
	GCPopSizeAAReducedMutSelModel* GetModel() {return (GCPopSizeAAReducedMutSelModel*) model;}

	GCPopSizeAAReducedMutSelSample(string filename, int inburnin, int inevery, int inuntil) : Sample(filename,inburnin,inevery,inuntil)	{
		Open();
	}
	...
	void Open()	{
		...
		if (modeltype == "GCPOPSIZEAAREDUCEDMUTSEL")	{
			model = new GCPopSizeAAReducedMutSelModel(datafile,treefile,P,withgc,withpopsize,true,type);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	GCPopSizeAAReducedMutSelSample sample(name,burnin,every,until);
	if (postpred)	{
