...
#include "MGOmegaGCCodonSubMatrix.h"
...
class MatrixTree : public BranchValPtrTree<RandomMGOmegaGCCodonSubMatrix>	{


	public:

	MatrixTree(CodonStateSpace* instatespace, RandomSubMatrix* innucmatrix, LengthTree* inpopsizetree, Var<Real>* inbgc, Var<PosReal>* inalpha, Var<PosReal>* inbeta) {
		SetWithRoot(true);
		...
		rootpopsize = new Const<PosReal>(1000.0);
	...
	int GetDiagStat()	{
		return GetDiagStat(GetRoot());
	}

	protected:

	int GetDiagStat(const Link* from)	{
		int tot = GetBranchVal(from->GetBranch())->GetDiagStat();
		for(const Link* link=from->Next(); link!=from; link=link->Next())	{
			tot += GetDiagStat(link->Out());
		}
		return tot;
	}

	RandomMGOmegaGCCodonSubMatrix* CreateBranchVal(const Link* link)	{
		if (link->isRoot())	{
			return new RandomMGOmegaGCCodonSubMatrix(statespace,nucmatrix,rootpopsize,bgc,alpha,beta);
		}
		return new RandomMGOmegaGCCodonSubMatrix(statespace,nucmatrix,popsizetree->GetBranchVal(link->GetBranch()),bgc,alpha,beta);
	}
...
class BranchMatrixPhyloProcess : public PhyloProcess	{
	...
	BranchMatrixPhyloProcess(LengthTree* intree, BranchValPtrTree<RandomMGOmegaGCCodonSubMatrix>* inmatrixtree,  SequenceAlignment* indata) : PhyloProcess(intree,indata)	{
	...
	protected:
	BranchValPtrTree<RandomMGOmegaGCCodonSubMatrix>* matrixtree;
};
...
class PopSizeModel : public ProbModel {
	...
	PopSizeModel(string datafile, string treefile, string contdatafile, bool sample=true, GeneticCodeType type=Universal)	{
		...
		matrixtree = new MatrixTree((CodonStateSpace*) codondata->GetStateSpace(), nucmatrix, popsizetree, bgc, alpha, beta);
		...
		RootRegister(matrixtree->GetRootPopSize());
	...
	*/
	...
	void MakeScheduler()	{
		...
		// scheduler.Register(new SimpleMove(popsizetree,10),30,"popsize");
		...
		scheduler.Register(new SimpleMove(bgc,1),30,"bgc");
		scheduler.Register(new SimpleMove(bgc,0.01),30,"bgc");
		scheduler.Register(new SimpleMove(bgc,0.001),30,"bgc");
		scheduler.Register(new SimpleMove(bgc,0.0001),30,"bgc");
	...
	void TraceHeader(ostream& os)	{
		os << "#logprior\tlnL\tlength\tbgc\tmeanlogpopsize\tvarlogpopsize\talpha\tbeta\tstatent\trrent\n";
	}
