...
class AAMutSelMixtureRandomMatrixTree : public MixtureRandomMatrixTree<Profile>	{
	...
	AAMutSelMixtureRandomMatrixTree(Var<Profile>* incenter, Var<PosReal>* inconcentration, CodonStateSpace* instatespace, BranchValPtrTree<RandomSubMatrix>* inmatrixtree, LengthTree* inpopsizetree)	{
		center = incenter;
...
class AAMutSelBranchMatrixFiniteMixture : public BranchMatrixFiniteMixture<Profile>	{
	...
	AAMutSelBranchMatrixFiniteMixture(int insize, int incomponentnumber, Var<Profile>* incenter, Var<PosReal>* inconcentration, CodonStateSpace* instatespace, BranchValPtrTree<RandomSubMatrix>* innucmatrixtree, LengthTree* inpopsizetree) :
	...
	MixtureRandomMatrixTree<Profile>* CreateComponent(int k)	{
		return new AAMutSelMixtureRandomMatrixTree(center,concentration,statespace,nucmatrixtree,popsizetree);
	}
...
class ACGTPopSizeAAMutSelModel : public ProbModel {
	...
	ACGTPopSizeAAMutSelModel(string datafile, string treefile, int inP, bool inwithgc, bool inwithpopsize, bool sample=true, GeneticCodeType type=Universal)	{
		...
		cerr << "mixture\n";
		P = inP;
		cerr << "number of components: " << P << '\n';

		concentration = new Exponential(NAA,Exponential::MEAN);
		center = new Dirichlet(Naa);

		if (withpopsize)	{
			aamutselmix = new AAMutSelBranchMatrixFiniteMixture(Nsite,P,center,concentration,codonstatespace,nucmatrixtree,popsizetree);
		}
		else	{
			aamutselmix = new AAMutSelBranchMatrixFiniteMixture(Nsite,P,center,concentration,codonstatespace,nucmatrixtree,0);
		}

		cerr << "create phylo process\n";
		phyloprocess = new BranchMatrixMixturePhyloProcess<Profile>(lognormaltree,aamutselmix,codondata);
	...
	MultiVariateTreeProcess* GetACGTProcess() {return process;}
	...
	*/
	...
	void MakeScheduler()	{
		...
		scheduler.Register(new AAMutSelBranchMatMixValMove(aamutselmix,1,1,40),1,"aamutsel mix subset");
		scheduler.Register(new AAMutSelBranchMatMixValMove(aamutselmix,0.3,2,40),1,"aamutsel mix subset");
		scheduler.Register(new AAMutSelBranchMatMixValMove(aamutselmix,0.1,3,40),1,"aamutsel mix subset");
		scheduler.Register(new AAMutSelBranchMatMixValMove(aamutselmix,0.03,4,40),1,"aamutsel mix subset");
		scheduler.Register(new BranchMatMixValMove<Profile>(aamutselmix,0.01,10),1,"aamutsel mix simple val");
		scheduler.Register(new BranchMatMixValMove<Profile>(aamutselmix,0.003,10),1,"aamutsel mix simple val");
	...
	void TraceHeader(ostream& os)	{
		os << "#logprior\tlnL\tlength\teffsize\tmeanent\tacgt\tvar\tlogpopsize\tvar\trrent\n";
	}
