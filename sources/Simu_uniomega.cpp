...
class Simulator : public NewickTree {
			...
			for (int gene=0; gene<Ngene; gene++)	{
				...
				for (int i=0; i<nsite; i++)	{
					...
					for (int final=0; final<Nnuc; final++)	{
						double tmp = epsilon * GetSubRate(init,final,prev,next,prev2,next2,phase,w2s,s2w);
						subrate[i][final] = tmp;
						totrate += tmp;
						totpos += tmp;
					}
	...
	double GetSubRate(int init, int final, int prev, int next, int prev2, int next2, int phase, double w2s, double s2w)	{
		double om = 1;
		if (phase == 0)	{
