...
class RandomBranchSitePath : public virtual Rnode, public BranchSitePath, public BranchSiteSubstitutionProcess	{
	...
	RandomBranchSitePath(PhyloProcess* inprocess) : myprocess(inprocess), propmatrix(0), matswap(false) {}
	...
	RandomBranchSitePath(PhyloProcess* inprocess, RandomTransitionMatrix* inmatrix, Var<Profile>* instationary) : myprocess(inprocess){
		propmatrix = 0;
		matrix = 0;
		length = 0;
		rate = 0;
	...
	void 			ResampleEnds(int instateup, int instatedown)	{
					stateup = instateup;
