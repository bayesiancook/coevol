...
class CombiCompoChain : public Chain	{
	...
	CombiCompoChain(string filename)	{
		contdatatype = 0;
		rrtype = "lg";
		name = filename;
		df = 2;
	...
	void New(int force)	{
		if (modeltype == "COMBICOMPO")	{
			model = new CombiCompoModel(datafile, treefile, contdatafile, calibfile, rootage, rootstdev, chronoprior, softa,  meanchi, meanchi2, df, clampdiag, contdatatype, rrtype, clamptree, meanexp, normalise, nrep, mix, rootfile, separatesyn, true);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	if (argc == 2)	{
		string name = argv[1];
		CombiCompoChain* chain = new CombiCompoChain(name);
		cerr << "start\n";
		cerr << '\n';
		chain->Start();
		cerr << '\n';
		...
		CombiCompoChain* chain = new CombiCompoChain(datafile, treefile, contdatafile, calibfile, rootage, rootstdev, chronoprior, softa, meanchi, meanchi2, df, clampdiag, contdatatype, rrtype, clamptree, meanexp, normalise, nrep, mix, rootfile, separatesyn, name, force);
