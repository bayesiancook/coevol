...
class CombiCompoSample : public Sample	{
	...
	CombiCompoModel* GetModel() {return (CombiCompoModel*) model;}

	CombiCompoSample(string filename, int inburnin, int inevery, int inuntil) : Sample(filename,inburnin,inevery,inuntil)	{
		Open();
	}
	...
	void Open()	{
		...
		if (modeltype == "COMBICOMPO")	{
			model = new CombiCompoModel(datafile, treefile, contdatafile, calibfile, rootage, rootstdev, chronoprior, softa,  meanchi, meanchi2, df, clampdiag, contdatatype, rrtype, clamptree, meanexp, normalise, nrep, mix, rootfile, separatesyn, false);
		}
		else	{
	...
	void Read(bool printlog, bool printmean, bool printci, bool printstdev, bool withleaf, bool withinternal)	{
		...
		int Ndata = GetModel()->Ndata;
		int* Kcomp = GetModel()->Kcomp;
		SumConstrainedMapping** map = GetModel()->changeofbasis;

		SumConstrainedMeanCovMatrix* mat = new SumConstrainedMeanCovMatrix(dim,Ndata,Kcomp,map);
...
int main(int argc, char* argv[])	{
	...
	CombiCompoSample sample(name,burnin,every,until);
