...
class OmegaMatrixTree : public BranchValPtrTree<RandomSubMatrix>	{


	public:

	OmegaMatrixTree(CodonStateSpace* instatespace, NucMatrixTree* innucmatrixtree, Var<PosReal>* inOmega)	{
		SetWithRoot(true);
	...
	RandomSubMatrix* CreateBranchVal(const Link* link)	{
		return new RandomMGOmegaCodonSubMatrix(statespace,nucmatrixtree->GetBranchVal(link->GetBranch()),omega);
	}
...
class BranchSiteMatrixPhyloProcess : public PhyloProcess	{
	
	protected:

	public:

	BranchSiteMatrixPhyloProcess(LengthTree* intree, OmegaMatrixTree** randmatrixtree,  SequenceAlignment* indata) : PhyloProcess(intree,indata)	{
		matrixtree = randmatrixtree;
	}
	...
	virtual RandomBranchSitePath* 	CreateRandomBranchSitePath(Link* link, int site)	{
		return  new RandomBranchSitePath(this,tree->GetBranchLength(link->GetBranch()),0,GetMatrix(site,link->GetBranch()),0);
	}

	RandomSubMatrix* 	GetMatrix(int site, const Branch* branch) {return matrixtree[site]->GetBranchVal(branch);}

	protected:
	OmegaMatrixTree** matrixtree;
};
...
class GCMixMGModel : public ProbModel	{
	...
	GammaMixture* omega;
	OmegaMatrixTree** omegamatrixtree;
	BranchSiteMatrixPhyloProcess* phyloprocess;
	...
	GCMixMGModel(string datafile, string treefile, int inK)	{
		...
		omegamatrixtree = new OmegaMatrixTree*[Nsite];
		for (int i=0; i<Nsite; i++)	{
			omegamatrixtree[i] = new OmegaMatrixTree(codonstatespace,nucmatrixtree,(*omega)[i]);
		}

		cerr << "create phylo process\n";
		phyloprocess = new BranchSiteMatrixPhyloProcess(gamtree,omegamatrixtree,codondata);
		cerr << "unfold\n";
		phyloprocess->Unfold();
		cerr << "register\n";
