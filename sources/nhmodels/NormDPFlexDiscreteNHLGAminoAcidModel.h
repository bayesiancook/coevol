...
class BinaryAllocationTree : public Rnode {
	...
	BinaryAllocationTree(BranchVarTree<RealVector>* indeltalogstattree, Var<UnitReal>* inswitchprob, Var<UnitReal>* instat, Var<RealVector>** inmeanvector, Var<PosReal>* invar)	{
		...
		stat = instat;
		meanvector = new Var<RealVector>*[2];
		meanvector[0] = inmeanvector[0];
		meanvector[1] = inmeanvector[1];
		var = invar;
	...
	double GetEmissionLogProb(const Link* from, int k, Var<RealVector>* deltalogstat)	{
		...
		for (int i=0; i<meanvector[0]->GetDim(); i++)	{
			double tmp = (*deltalogstat)[i] - (*(meanvector[k]))[i];
			total += tmp * tmp;
		}
		double tmp = -0.5 * meanvector[0]->GetDim() * log(var->val()) -0.5 * total / var->val();
		return tmp;
		// return exp(tmp);
	}
...
class DeltaLogStatTree : public BranchProcess<RealVector> {
	...
	double RecursiveGetMean(const Link* from, int index, int& tot)	{
		double total = (*GetIIDNormal(from))[index];
		tot++;
		for(Link* link=from->Next(); link!=from; link=link->Next())	{
...
class DiscreteStat : public Dvar<Profile>	{
	...
	DiscreteStat(Var<RealVector>** inlist, int inP)	{
		setval(Profile(inlist[0]->GetDim()));
		bkvalue = *this;
		P = inP;
...
class DiscreteStatTree : public BranchValPtrTree<Dvar<Profile> > {
	...
	DiscreteStatTree(DeltaLogStatTree** inlist, int inP)	{
		SetWithRoot(true);
		P = inP;
	...
	Dvar<Profile>* CreateBranchVal(const Link* link)	{
		...
		DiscreteStat* ret = new DiscreteStat(tmp,P);
		delete[] tmp;
		return ret;
	}
...
class FlexDiscreteNHAminoAcidModel: public ProbModel {
	...
	Gamma* varlogstat;
	NormalIIDArray* deltalogstat0;
	NormalIIDArray* deltalogstat1;
	...
	FlexDiscreteNHAminoAcidModel(string datafile, string treefile, string contdatafile, bool inwithsep, bool sample=true, GeneticCodeType type=Universal)	{
		...
		deltalogstat0 = new NormalIIDArray(Naa,Ncont,Zero,One);
		deltalogstat1 = new NormalIIDArray(Naa,Ncont,Zero,One);
		deltalogstattree = new DeltaLogStatTree*[Ncont];
		for (int p=0; p<Ncont; p++)	{
			deltalogstattree[p] = new DeltaLogStatTree(tree,Naa);
			Var<RealVector>** tmp = new Var<RealVector>*[2];
			tmp[0] = (*deltalogstat0)[p];
			tmp[1] = (*deltalogstat1)[p];
			alloctree[p] = new BinaryAllocationTree(deltalogstattree[p],(*binswitch)[p],(*binstat)[p],tmp,varlogstat);
		}

		discretestattree = new DiscreteStatTree(deltalogstattree,Ncont);
	...
	double GetCenter(int k, int i, int p=0)	{
		if (k == 1)	{
			double total = 0;
			for (int j=0; j<Naa; j++)	{
				total += exp((*(*deltalogstat1)[p])[j]);
			}
			return exp((*(*deltalogstat1)[p])[i]) / total;
		}
		double total = 0;
		for (int j=0; j<Naa; j++)	{
			total += exp((*(*deltalogstat0)[p])[j]);
		}
		return exp((*(*deltalogstat0)[p])[i]) / total;
	}
		
	BinaryAllocationTree* GetAllocationTree(int k=0)	{return alloctree[k];}
	...
	*/
	...
	double GetLogPrior()	{
		...
		total += deltalogstat1->GetLogProb();
		total += deltalogstat0->GetLogProb();
		for (int p=0; p<Ncont; p++)	{
	...
	void MakeScheduler()	{
		...
		scheduler.Register(new NormalIIDArrayMove(deltalogstat0,0.1,2),100,"delta log stat");
		scheduler.Register(new NormalIIDArrayMove(deltalogstat0,0.03,2),100,"delta log stat");
		scheduler.Register(new NormalIIDArrayMove(deltalogstat0,0.01,5),100,"delta log stat");
		scheduler.Register(new NormalIIDArrayMove(deltalogstat0,0.001,10),100,"delta log stat");

		scheduler.Register(new NormalIIDArrayMove(deltalogstat1,0.1,2),100,"delta log stat");
		scheduler.Register(new NormalIIDArrayMove(deltalogstat1,0.03,2),100,"delta log stat");
		scheduler.Register(new NormalIIDArrayMove(deltalogstat1,0.01,5),100,"delta log stat");
		scheduler.Register(new NormalIIDArrayMove(deltalogstat1,0.001,10),100,"delta log stat");

		for (int p=0; p<Ncont; p++)	{
			scheduler.Register(new DeltaLogStatTreeMove(deltalogstattree[p],1,2),3,"delta log stat tree");
			scheduler.Register(new DeltaLogStatTreeMove(deltalogstattree[p],0.3,2),3,"delta log stat tree");
			scheduler.Register(new DeltaLogStatTreeMove(deltalogstattree[p],0.1,5),3,"delta log stat tree");
			scheduler.Register(new DeltaLogStatTreeMove(deltalogstattree[p],0.01,10),3,"delta log stat tree");
		}

		scheduler.Register(new SimpleMove(binswitch,1),100,"binary switch");
		scheduler.Register(new SimpleMove(binswitch,0.1),100,"binary switch");
		scheduler.Register(new SimpleMove(binswitch,0.01),100,"binary switch");

		scheduler.Register(new SimpleMove(binstat,1),100,"binary stat");
		scheduler.Register(new SimpleMove(binstat,0.1),100,"binary stat");
		scheduler.Register(new SimpleMove(binstat,0.01),100,"binary stat");

		scheduler.Register(new SimpleMove(varlogstat,1),100,"var log stat");
		scheduler.Register(new SimpleMove(varlogstat,0.1),100,"var log stat");
		scheduler.Register(new SimpleMove(varlogstat,0.01),100,"var log stat");
	...
	void drawSample()	{
		...
		*/
		deltalogstat0->Sample();
		deltalogstat0->SetAtZero();
		deltalogstat1->Sample();
		deltalogstat1->SetAtZero();
		for (int p=0; p<Ncont; p++)	{
			cerr << p << '\n';
	...
	void TraceHeader(ostream& os)	{
		os << "#logprior\tlnL\tlength\talpha\tvarlogstat";
		for (int p=0; p<Ncont; p++)	{
			os << "\tswitch" << p;
			os << "\tstat" << p;
			os << "\ttotalloc" << p;
	...
	void Trace(ostream& os)	{
		...
		for (int p=0; p<Ncont; p++)	{
			...
			os << '\t' << (*deltalogstat0)[p]->GetMean();
			os << '\t' << (*deltalogstat0)[p]->GetVar();
			os << '\t' << (*deltalogstat1)[p]->GetMean();
			os << '\t' << (*deltalogstat1)[p]->GetVar();
			os << '\t' << deltalogstattree[p]->GetGrandMean();
			os << '\t' << deltalogstattree[p]->GetGrandVar();
			os << '\t' << (*(*deltalogstat0)[p])[0];
			os << '\t' << (*(*deltalogstat1)[p])[0];
			os << '\t' << deltalogstattree[p]->GetMean(0);
		}
		os << '\n';
		os.flush();
	}
