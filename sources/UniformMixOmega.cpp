...
class MixOmegaChain : public Chain	{
	...
	void New(int force)	{
		if (modeltype == "UNIMIXOMEGA")	{
			model = new MixOmegaModel(datafile,treefile,contdatafile,calibfile,rootage,rootstdev,chronoprior,meanchi,meanchi2,df,clampdiag,conjpath,clamptree,meanexp,normalise,nrep,suffstatfile,rootfile,clampreg,mappingfreq,mappingevery,omegafile,type,myid,nprocs,true);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	if (argc == 2)	{
		...
		MixOmegaChain* chain = new MixOmegaChain(datafile,treefile,contdatafile,calibfile,rootage,rootstdev,chronoprior,meanchi,meanchi2,df,type,name,clampdiag,conjpath,clamptree,meanexp,normalise,nrep,suffstatfile,rootfile,clampreg,mappingfreq,mappingevery,omegafile,myid,nprocs,force);
