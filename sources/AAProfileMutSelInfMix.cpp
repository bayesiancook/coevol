...
class AAProfileMutSelInfMixChain : public Chain	{
	...
	AAProfileMutSelInfMixChain(string indata, string intree, int inP, string filename, GeneticCodeType intype, int inevery = 1, int inuntil = -1, int force = 1)	{
		modeltype = "AAPROFILEINFMIXMUTSEL";
		datafile = indata;
		treefile = intree;
		type = intype;
		P = inP;
	...
	AAProfileMutSelInfMixChain(string filename)	{
		name = filename;
		Open();
	}
	...
	void Open()	{
		...
		if (modeltype == "AAPROFILEINFMIXMUTSEL")	{
			model = new AAProfileMutSelMatInfMixModel(datafile,treefile,P,false,type);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	AAProfileMutSelInfMixChain* chain;
	if (datafile == "")	{
		// This is an already existing chain on the disk; reopen and restart
		chain = new AAProfileMutSelInfMixChain(name);
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
	}
		...
		// This is a new chain.
		//
		chain = new AAProfileMutSelInfMixChain(datafile,treefile,P,name,type,every,until);
		chain->Gnuplot();
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
