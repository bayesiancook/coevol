...
class LogPopEffOmega : public Dvar<Real>	{

	public:

	LogPopEffOmega(Var<Real>* inalpha, Var<Real>* inbeta, Var<Real>* inlogpopeff)	{
		alpha = inalpha;
		beta = inbeta;
	...
	void specialUpdate()	{
		setval(beta->val() + alpha->val() * logpopeff->val());
	}
...
class LogOmegaTree : public NodeValPtrTree<Dvar<Real> >	{
	...
	LogOmegaTree(NodeVarTree<Real>* inpopefftree, Var<Real>* inalpha, Var<Real>* inbeta) {
	...
	Dvar<Real>* CreateNodeVal(const Link* link)	{
		return new LogPopEffOmega(alpha, beta, popefftree->GetNodeVal(link->GetNode()));
	}
