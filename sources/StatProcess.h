...
#ifndef STATPROCESS_H
#define STATPROCESS_H

class InstantStat : public Dvar<Profile>	{

	public:

	InstantStat(Var<Profile>* inrefstat, Var<RealVector>* inlogval, int inoffset)	{
		setval(Profile(inrefstat->GetDim()));
		bkvalue = Profile(inrefstat->GetDim());
		refstat = inrefstat;
		logval = inlogval;
		offset = inoffset;
		Register(refstat);
	...
	void specialUpdate()	{
		...
		for (int i=0; i<GetDim(); i++)	{
			(*this)[i] = (*refstat)[i] * exp((*logval)[i + offset]);
			total += (*this)[i];
		}
		for (int i=0; i<GetDim(); i++)	{
			(*this)[i] /= total;
...
class StatTree : public NodeValPtrTree<Dvar<Profile> >	{
	...
	StatTree(Var<Profile>* inrefstat, NodeVarTree<RealVector>* inprocess, int inoffset)	{
		offset = inoffset;
		process = inprocess;
		refstat = inrefstat;
	...
	Dvar<Profile>* CreateNodeVal(const Link* link)	{
		return new InstantStat(refstat,process->GetNodeVal(link->GetNode()),offset);
	}
