...
class AAProfileMutSelChain : public Chain	{
	...
	AAProfileMutSelChain(string indata, string intree, int inP, string filename, GeneticCodeType intype, string inprofilefile, int inevery = 1, int inuntil = -1, int force = 1)	{
		modeltype = "AAPROFILEMUTSEL";
		datafile = indata;
		treefile = intree;
		type = intype;
		P = inP;
	...
	AAProfileMutSelChain(string filename)	{
		name = filename;
		Open();
	}
	...
	void Open()	{
		...
		if (modeltype == "AAPROFILEMUTSEL")	{
			model = new AAProfileMutSelModel(datafile,treefile,P,false,type,profilefile);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	if (datafile == "")	{
		...
		// This is a new chain.
		chain = new AAProfileMutSelChain(datafile,treefile,P,name,type,profilefile,every,until);
		chain->Gnuplot();
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
