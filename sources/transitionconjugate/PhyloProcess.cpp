...
void PhyloProcess::Unfold()	{
	...
	GetTree()->RecursiveCreateTBL(GetRoot(),GetMaxNstate());
...
void PhyloProcess::Cleanup()	{
	GetTree()->RecursiveDeleteTBL(GetRoot());
	...
	if (from->isLeaf())	{
		...
		for(const Link* link=from->Next(); link!=from; link=link->Next())	{
			if (! isMissing(link->GetBranch(),site))	{
				...
				GetPath(link->GetBranch(),site)->BackwardPropagate(link->Out()->tbl,link->tbl);
				// GetBranchSiteSubstitutionProcess(link->GetBranch(),site)->BackwardPropagate(link->Out()->tbl,link->tbl);
				for (int k=0; k<GetNstate(site); k++)	{
					t[k] *= tbl[k];
				}
