...
class TransitionPathConjugate : public DSemiConjugatePrior<void>	{
	...
	TransitionPathConjugate(RandomTransitionMatrix* inmatrix, bool inisroot)	{
		matrix = inmatrix;
		isroot = inisroot;
		Register(matrix);
		CreateSuffStat();
...
class TransitionPathConjugateTree : public BranchValPtrTree<TransitionPathConjugate>	{
...
class BranchMatrixTransitionPathConjugateTree : public TransitionPathConjugateTree	{
	...
	TransitionPathConjugate* CreateBranchVal(const Link* link)	{
		return new TransitionPathConjugate(matrixtree->GetBranchVal(link->GetBranch()), link->isRoot());
	}
...
class DSemiConjugateMappingMove : public MCUpdate	{
	...
	DSemiConjugateMappingMove(PhyloProcess* inprocess, TransitionPathConjugateTree* inpathconjtree, double intuning = 1) : process(inprocess), pathconjtree(inpathconjtree) {
		tuning  = intuning;
	}
