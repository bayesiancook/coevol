...
class CodonProfileMutSelInfMixChain : public Chain	{
	...
	CodonProfileMutSelInfMixChain(string indata, string intree, int inP, string filename, GeneticCodeType intype, int inevery = 1, int inuntil = -1, int force = 1)	{
		modeltype = "CODONPROFILEINFMIXMUTSEL";
		datafile = indata;
		treefile = intree;
		type = intype;
		P = inP;
	...
	CodonProfileMutSelInfMixChain(string filename)	{
		name = filename;
		Open();
	}
	...
	void Open()	{
		...
		if (modeltype == "CODONPROFILEINFMIXMUTSEL")	{
			model = new CodonProfileMutSelMatInfMixModel(datafile,treefile,P,false,type);
		}
		else	{
...
int main(int argc, char* argv[])	{
	...
	CodonProfileMutSelInfMixChain* chain;
	if (datafile == "")	{
		// This is an already existing chain on the disk; reopen and restart
		chain = new CodonProfileMutSelInfMixChain(name);
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
	}
		...
		// This is a new chain.
		//
		chain = new CodonProfileMutSelInfMixChain(datafile,treefile,P,name,type,every,until);
		chain->Gnuplot();
		cerr << "start\n";
		chain->Start();
		cerr << "exit\n";
